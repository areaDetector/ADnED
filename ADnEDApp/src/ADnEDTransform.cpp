 ...
 * Transform types are:
 *
 * 0 - multiply the TOF by a pixel ID lookup in doubleArray[0]. Can be used
 *     for fixed geometry instruments to calculate dspace. Can be used on Vulcan for example.
 *
 * 1 - calculate dspace where the theta angle changes. Used for direct geometry 
 *     instruments like Hyspec. NOTE: not sure how this works yet.  
 *
 * 2 - calculate deltaE for indirect geometry instruments for their inelastic detectors. 
 *     This uses an equation which depends on the mass of the neutron, L1, an array of L2
 *     and an array of Ef. The final energy per-pixelID, Ef, is known because of the use of 
 *     mirrors to select energy. We can calculate incident energy using the known final 
 *     energy and the TOF. So we are calculating energy transfer for each event.
 ...
 * 
 *     One the deltaE has been obtained in Joules, it is converted back to eV. 
 */
